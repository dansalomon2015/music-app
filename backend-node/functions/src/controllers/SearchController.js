    ...
    search(req, res, next) {
        try {
            const { q } = yup
                ...
                .validateSync(req.query, types_1.DATA_VALIDATION_OPTIONS);
            (0, thirdParties_1.search)(q)
                .then((data) => {
                let response = new utils_1.ApiResponse("Success", types_1.ApiResponseCodeType.SUCCESS);
                response.setData(data.data.map((r) => mappers_1.SearchResultMapper.mapResponseModel(r)));
